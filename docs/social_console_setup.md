...
flutter run \
  ...
  --dart-define=KAKAO_REDIRECT_URI=photoshield://kakao-auth \
...
3. Configure valid OAuth redirect URIs for your mobile callback scheme.
...
3. Register the native app key and callback URI used by the app.
...
2. Register Android/iOS app identifiers and the callback URI.
...
The unified monitoring pipeline in the Flutter app merges available Meta scans
with Kakao/Naver placeholders. Production-grade Kakao/Naver media discovery,
reverse image matching, and provider token exchange should be implemented in a
backend service so secrets and platform-specific API constraints stay off the client.
